...
def main():
    ...
    NUM_PROF_FNS = 12  # full forward pass phases
    ...
    MAX_ITERS = 64
    n_iter_f32 = MAX_ITERS * 2  # 2 f32 per timestamp
    ...
    c_total = cycles.flatten()  # raw total cycles (not divided by repeat_steps)
    ...
    # Reconstruct 48-bit TSC values from packed f32 pairs, compute per-iter deltas
    def reconstruct_u48(u32_pair):
        """Reconstruct 48-bit TSC from 2 packed u32 values (bitcast from f32)."""
        lo32 = int(u32_pair[0])
        hi16 = int(u32_pair[1]) & 0xFFFF
        return lo32 + (hi16 << 32)

    n_iters_to_show = min(repeat_steps, MAX_ITERS)
    # Also reconstruct tsc_end for the final boundary
    def reconstruct_tsc_end(timer_3f32):
        """Reconstruct tsc_end from the 3-f32 timer_buf (start[0:1], start[2]|end[0], end[1:2])."""
    ...
    # Compute per-iteration cycles for each PE, then aggregate
    iter_cycles = np.zeros((P, P, n_iters_to_show), dtype=np.float64)
    for pe_y in range(P):
        for pe_x in range(P):
            # Reconstruct boundary timestamps (one per iteration start + tsc_end)
            boundaries = []
            for i in range(n_iters_to_show):
                boundaries.append(reconstruct_u48(iter_hwl[pe_y, pe_x, i*2:(i+1)*2]))
            boundaries.append(reconstruct_tsc_end(timer_hwl[pe_y, pe_x, :]))
            for i in range(n_iters_to_show):
