...
def main():
    ...
    runner.memcpy_h2d(
        ...
        # Copy K_weight (GQA: packed for all KV heads, each PE stores all KV-head blocks)
    # tensor_k_weight: [n_kv_heads, dim, dim]
    k_blocks = []
    ...
    # Pack along the last dimension: [P, P, n_kv_heads * block]
    K_packed = np.concatenate(k_blocks, axis=2)
    K_u32 = input_array_to_u32(K_packed.ravel(), 1, 1)
    runner.memcpy_h2d(
        sym_K_weight, K_u32, 0, 0, P, P, n_kv_heads * dim_p_pe * dim_p_pe,
        streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
    )

# Copy V_weight
    # Copy V_weight (GQA: packed for all KV heads, each PE stores all KV-head blocks)
    v_blocks = []
    for h in range(n_kv_heads):
    ...
    V_packed = np.concatenate(v_blocks, axis=2)
    V_u32 = input_array_to_u32(V_packed.ravel(), 1, 1)
    runner.memcpy_h2d(
        sym_V_weight, V_u32, 0, 0, P, P, n_kv_heads * dim_p_pe * dim_p_pe,
        streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
    )
    ...
    # Copy XKCache (GQA: packed for all KV heads, each PE stores all KV-head blocks)
    xk_blocks = []
    for h in range(n_kv_heads):
    ...
    XKCache_packed = np.concatenate(xk_blocks, axis=2)
    XKCache_u32 = input_array_to_u32(XKCache_packed.ravel(), 1, 1)
    runner.memcpy_h2d(
        sym_XKCache, XKCache_u32, 0, 0, P, P, n_kv_heads * dim_p_pe * seq_len_p_pe,
        streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
    )

    # Copy XVCache (GQA: packed for all KV heads, each PE stores all KV-head blocks)
    xv_blocks = []
    for h in range(n_kv_heads):
    ...
    XVCache_packed = np.concatenate(xv_blocks, axis=2)
    XVCache_u32 = input_array_to_u32(XVCache_packed.ravel(), 1, 1)
    runner.memcpy_h2d(
        sym_XVCache, XVCache_u32, 0, 0, P, P, n_kv_heads * seq_len_p_pe * dim_p_pe,
        streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
    )
